...
| `~/.npmrc`, `~/.netrc` | `<target>/secrets/bundle.zip.age` (+ Drive) — **procedure:** `backup/manual/secrets.md` |
...
5. Run `backup --secrets` (or `offsite-secrets.sh --with-gpg`) on each Mac — see `backup/manual/secrets.md`
